...
const NovelChapterCreation = () => {
  ...
  const fetchAllStory = async () => {
    ...
    try {
      const { userToken, user } = JSON.parse(localStorage.getItem("user_data"));
      const { data } = await axiosPrivate.get("story/list/lightnovel", {
        headers: {
          Authorization: `Bearer ${userToken}`,
        },
      });
          ...
          <div className="update-story-item-popup">
            ...
            <div style={{ width: "100%" }}>
              <TextArea
                rows={10}
                value={bodyChapter}
                onChange={(e) => setBodyChapter(e.target.value)}
              />
            </div>
          </div>
        </Modal>
...
export default NovelChapterCreation;
